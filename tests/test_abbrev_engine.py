...
# ═══ AB-04：設定檔暫時鎖住不覆寫自訂縮寫 / 損壞旗標 ═══════════════════════════
def _write_custom_abbrev(p):
    import json as _json
    custom = {"schema_version": ae.ABBREV_CONFIG_SCHEMA_VERSION, "enabled": True,
              "items": [{"abbrev": "myabbr", "expansion": "my custom text"}]}
    p.write_text(_json.dumps(custom), encoding="utf-8")


def test_ab04_transient_lock_first_load_sets_load_failed(tmp_path, monkeypatch):
    """[codex P1] 首次載入即持續鎖 → load_failed=True、不覆寫、不快取(main.py 據此不快取)。"""
    p = tmp_path / "abbrev.json"
    _write_custom_abbrev(p)
    before = p.read_bytes()
    ae._LAST_GOOD_CONFIG.clear()
    monkeypatch.setattr(ae.time, "sleep", lambda s: None)
    monkeypatch.setattr(ae, "safe_load_json_ex",
                        lambda path, default=None: (default, "error"))
    cfg = ae.load_config(str(p))
    assert p.read_bytes() == before        # [AB-04] 檔案未被預設值覆寫
    assert cfg.load_failed is True


def test_ab04_transient_lock_reuses_last_good(tmp_path, monkeypatch):
    """[codex P1] 先成功載入(快取)→ 之後暫時鎖 → 沿用上次真設定(非預設)、不覆寫。"""
    p = tmp_path / "abbrev.json"
    _write_custom_abbrev(p)
    ae._LAST_GOOD_CONFIG.clear()
    good = ae.load_config(str(p))          # 成功 → 快取
    assert any(it["abbrev"] == "myabbr" for it in good.items)
    before = p.read_bytes()
    monkeypatch.setattr(ae.time, "sleep", lambda s: None)
    monkeypatch.setattr(ae, "safe_load_json_ex",
                        lambda path, default=None: (default, "error"))
    cfg = ae.load_config(str(p))
    assert any(it["abbrev"] == "myabbr" for it in cfg.items)   # 沿用上次真設定
    assert cfg.load_failed is False
    assert p.read_bytes() == before


def test_ab04_corrupt_sets_recovered_flag(tmp_path):
    import json as _json
    p = tmp_path / "abbrev.json"
    p.write_text("{bad json", encoding="utf-8")
    cfg = ae.load_config(str(p))
    assert cfg.recovered_from_corrupt is True
    # 正常檔不設旗標
    p.write_text(_json.dumps({"enabled": True, "items": []}), encoding="utf-8")
    assert ae.load_config(str(p)).recovered_from_corrupt is False


def test_ab04_recovered_flag_not_persisted():
    """recovered_from_corrupt 不進 to_dict、不參與相等比較。"""
    cfg = AbbrevConfig(enabled=True)
    cfg.recovered_from_corrupt = True
    cfg.load_failed = True
    assert "recovered_from_corrupt" not in cfg.to_dict()
    assert "load_failed" not in cfg.to_dict()
    assert AbbrevConfig(enabled=True) == AbbrevConfig(enabled=True)


# ═══ AB-05：剪貼簿還原兩缺口 ═══════════════════════════════════════════════
def test_ab05b_open_failed_forces_keystroke(monkeypatch):
    """[AB-05b] 開剪貼簿失敗 → 完全不碰剪貼簿(不呼叫 set)、改走 keystroke。"""
    eng = _make_engine()
    monkeypatch.setattr(ae, "_replace_native_edit_suffix",
                        lambda *a, **k: ae._NATIVE_NOT_APPLICABLE)
    monkeypatch.setattr(ae, "_clipboard_get_text", lambda: ae._CLIP_OPEN_FAILED)
    set_calls = []
    monkeypatch.setattr(ae, "_clipboard_set_text",
    ...
    eng._interleaved = False
    eng._do_replace(3, "dermatofibroma", "df", "df ", 0, 0)
    assert set_calls == []                 # 不碰剪貼簿
    assert ("write", "dermatofibroma") in eng._kb.sent


def test_ab05a_set_text_accepts_restore_on_fail():
    """[AB-05a] _clipboard_set_text/impl 接受 restore_on_fail;回寫 helper 存在。"""
    import inspect
    assert "restore_on_fail" in inspect.signature(ae._clipboard_set_text).parameters
    assert "restore_on_fail" in inspect.signature(
        ae._clipboard_set_text_impl).parameters
    assert callable(ae._set_clipboard_data_within_open)
