...
# ── [2026-07-16 撤回] 打卡表格存在性【不可】當 portal 改版信號 ──────────────────
def test_empty_swipe_table_not_treated_as_portal_change():
    # [GPT-5.6 P0 regression] 空的 ASP.NET GridView(當日尚無打卡)本來就不渲染 <table>
    # → 「表格不存在」＝今天未打卡,不是改版。絕不可在此回錯誤/portal_changed,否則早上
    #  未打卡時會灰燈+不重試、隱藏最重要訊號。守門:該路徑不得再用表格存在性判改版。
    assert not hasattr(main, "CLOCK_ERR_PORTAL_CHANGED"), \
        "打卡改版偵測(表格存在性)已撤回,常數應移除"
    src = inspect.getsource(main._get_swipe_status_from_web)
    assert "table_present" not in src and "portal_changed" not in src.lower(), \
        "不得再以表格存在性當改版信號(空表＝未打卡)"
    # 仍以 querySelectorAll 掃列(表格不存在→空 NodeList→0 列→正常顯示未打卡)
    assert 'querySelectorAll("#Gv_attppre tbody tr")' in src
