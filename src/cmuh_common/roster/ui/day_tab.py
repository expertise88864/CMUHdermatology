...
_SPECIAL_SLOTS = frozenset((TREATMENT, BIOPSY, REST))


def _rooms_summary(slots: dict) -> str:
    """把房號格彙整成 '101:AB 102:C'（治療室/切片室/放假為特殊格，不算房）。
...
class _DayEditDialog(tk.Toplevel):
    """手動改某日某時段的各格（治療室/切片室/房號/放假），逗號分隔代號。"""
    ...
    def __init__(self, master, service, ym, d, session, on_done):
        ...
        self._slots = [TREATMENT, BIOPSY, *sorted({*rooms, *stale}), REST]
        self._entries: dict = {}
